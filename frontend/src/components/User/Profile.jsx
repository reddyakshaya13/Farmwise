...
const Profile = () => {
    ...
    return (
        <>
            <MetaData title={`${user?.name} (@${user?.username}) • Farmwise photos and videos`} />
                ...
                <div className="mt-16 xl:w-2/3 mx-auto">
                                ...
                                {(loggedInUser.username === user.username) ? (
                                    ...
                                    <div className="flex gap-3 items-center">
                                        {follow ? (
                                            ...
                                            <button onClick={handleFollow} className="font-medium bg-forest-green text-sm text-white hover:shadow rounded px-6 py-1.5">Follow</button>
                                        )}
                    ...
                    <div className="border-t sm:ml-8 sm:mr-14">
                        ...
                        <div className="flex gap-12 justify-center">
                            <span onClick={() => setSavedTab(false)} className={`${savedTab ? 'text-gray-400' : 'border-t border-green-500'} py-3 cursor-pointer flex items-center text-[13px] uppercase gap-3 tracking-[1px] font-medium`}>
                                {savedTab ? postsIconOutline : postsIconFill} posts</span>
                            {user._id === loggedInUser._id && (
                                <span onClick={() => setSavedTab(true)} className={`${savedTab ? 'border-t border-green-500' : 'text-gray-400'} py-3 cursor-pointer flex items-center text-[13px] uppercase gap-3 tracking-[1px] font-medium`}>
                        ...
                        {savedTab ?
                            ...
                            user?.posts?.length > 0 ?
                                ...
                                <div className="bg-white mt-2 mb-10 drop-shadow-sm rounded flex sm:flex-row flex-col sm:gap-0 gap-5 sm:p-0 p-4 items-center justify-between">
                                    <img draggable="false" className="w-2/5 rounded-l" src={require(`../../assests/images/logos/favicon.png`)} alt="" />
