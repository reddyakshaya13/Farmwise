    ...
    return (
        <nav className="fixed top-0 w-full border-b bg-white z-10">
            ...
            <div className="flex flex-row justify-between items-center py-2 px-3.5 sm:w-full sm:py-2 sm:px-4 md:w-full md:py-2 md:px-6 xl:w-4/6 xl:py-3 xl:px-8 mx-auto">
                ...
                {/* <!-- logo --> */}
                <Link to="/"><img draggable="false" className="mt-1.5 w-full h-full object-contain" src={require(`../../assests/images/logos/favicon.png`)} alt="" /></Link>
                ...
                <div className="flex items-center space-x-6 sm:mr-5">
                    ...
                    <Link to="/articles">{exploreOutline}</Link>

                    <div onClick={() => setProfileToggle(!profileToggle)} className={`${profileToggle && 'border-green-500 border' || (!onHome && !onChat) && 'border-green-500 border'} rounded-full cursor-pointer h-7 w-7 p-[0.5px]`}><img draggable="false" loading="lazy" className="w-full h-full rounded-full object-cover" src={user.avatar} alt="" /></div>
                </div>
